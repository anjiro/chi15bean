...
app.intervals = [];   //list of interval timers to clear on disconnect
	...
	function onScanSuccess(device) {
		...
		function onConnectSuccess(device) {
			...
			function onServiceSuccess(device) {
				...
				document.getElementById('BLEButton').onclick = new Function('app.disconnect(user)');
...
app.disconnect = function(user) {
	...
	//If timers configured, clear. 
	app.intervals.forEach(function(interval){clearInterval(interval)});
...
privateKey = 'kzDBoEDgeMteyzMRVlBp';
publicKey  = 'mK4qwb4XyphymYpXGoRd';

app.sparkWrite = function(uid, key, value)
{
	$.post(
...
app.sparkRead = function(callback)
{
	var url = "https://data.sparkfun.com/output/" + publicKey + '.json';
